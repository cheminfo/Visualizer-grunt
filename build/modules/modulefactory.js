define(['jquery', 'modules/module'], function($, Module) {
	...
	function getSubFoldersFrom( folder ) {
		...
		return $.getJSON( folder , {}, function( data ) {
			getModules( data );
		});
	}
	...
	function getModules( folderInfo ) {
		...
		for( var i in folderInfo.folders ) {
			...
			( function( j ) {
				...
				modulesDeferred.push( getSubFoldersFrom( folderInfo.folders[ i ] + "folder.json" ).done( function( folder ) {
					folderInfo.folders[ i ] = folder;	
				} ) );
